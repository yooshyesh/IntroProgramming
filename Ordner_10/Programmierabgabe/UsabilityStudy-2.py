...
class UsabilityStudy(): # represents a collection of many objects and the logic behind managing them
    ...
    def load_from_json(self, filepath):
        with open(filepath, "r") as file:
            data = json.load(file)
        for user_id, entry in enumerate(data, start=1): # go through each line and entry in json file
    ...
    def save_evaluation(self, filepath):
        ...
        with open(filepath, "w", encoding="utf-8") as file:
            file.write(f"Erfolgreiche Nutzer:innen: {successful}")
            file.write(f"Durchschnittliche Dauer: int({avg_duration})")
            file.write(f"Nutzer:innen mit >2 Backtracks und Erfolg:\n{filtered_u}")
            for session in filtered_u:
